                ...
                switch (button.customId) {
                    case 'rel':
                        const embedd = new MessageEmbed()
                            ...
                            .setThumbnail(client.user.displayAvatarURL())
                            .setTitle(client.user.username)
                            .setFooter({ text: 'Music Bot By N2.DUC', iconURL: message.author.avatarURL({ dynamic: true }) })
                            .setDescription(`**
            > Guilds: \`${client.guilds.cache.size}\`
            > Users: \`${Math.ceil(client.guilds.cache.reduce((a, b) => a + b.memberCount, 0).toLocaleString("tr-TR"))}.000\`
            > Channels: \`${client.channels.cache.size}\`**`)
                        ...
                        button.reply({ content: "> **✅ Thành công: ** Đã cập nhật số liệu thống kê về bot!", ephemeral: true }).catch(e => {});
